...
export default function CarrerasCalecita() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      data.sort((a, b) => new Date(a.fechaCarrera).getTime() - new Date(b.fechaCarrera).getTime());
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div
            ...
            {carreraAnterior ? (
              <>
                <div className="pb-2 relative z-10">
                  ...
                  <div className="text-gray-400 text-sm text-center">
                    {carreraAnterior.fechaCarrera} - {carreraAnterior.horaCarrera}
                  </div>
                  <div className="text-gray-300 text-center mt-1">
                    {carreraAnterior.circuito?.nombre} ({carreraAnterior.circuito?.pais})
                  </div>
                </div>
                <div className="relative z-10 mt-4">
                  <h4 className="text-white font-semibold text-center mb-1">Resultados</h4>
                  {carreraAnterior.resultados && carreraAnterior.resultados.length > 0 ? (
                    <ol className="text-gray-100 text-center text-base">
                      {carreraAnterior.resultados.slice(0, 3).map((piloto, idx) => (
                        <li key={idx}>
                          {idx === 0 && "🥇"}
                          {idx === 1 && "🥈"}
                          {idx === 2 && "🥉"}{" "}
              ...
              <div className="text-gray-400 text-center mt-10">No hay carrera anterior</div>
            )}
            {/* Decoracion */}
          ...
          <div
            className="min-h-72 p-10 m-1 relative overflow-hidden border-2 border-red-400/70 hover:border-red-500/80 transition-all duration-500 shadow-2xl shadow-red-500/20 transform hover:-translate-y-2 bg-gradient-to-br from-red-600/40 to-black/60 backdrop-blur-md flex flex-col justify-between scale-115">
            {/* Decoracion */}
            <div className="absolute inset-0 opacity-20">
            ...
            {carreraCentral ? (
              <>
                <div className="pb-2 relative z-10">
                  ...
                  <div className="text-gray-200 text-center mb-2">
                    {carreraCentral.circuito?.nombre} ({carreraCentral.circuito?.pais})
                  </div>
                  <div className="flex flex-col items-center gap-1 mb-2">
                    <span className="text-gray-300 text-sm"><b>Ubicación:</b> {carreraCentral.circuito?.ubicacion}</span>
                    <span className="text-gray-300 text-sm"><b>Vueltas:</b> {carreraCentral.circuito?.vueltas}</span>
                    <span className="text-gray-300 text-sm"><b>Longitud:</b> {carreraCentral.circuito?.longitud_km} km</span>
                  </div>
                </div>
              ...
              <div className="text-gray-400 text-center mt-10">No hay carrera próxima</div>
            )}
            {/* Decoracion */}
            ...
            {carreraSiguiente ? (
              <>
                <div className="pb-2 relative z-10">
                  ...
                  <div className="text-gray-400 text-sm text-center">
                    {carreraSiguiente.fechaCarrera} - {carreraSiguiente.horaCarrera}
                  </div>
                  <div className="text-gray-300 text-center mt-1">
              ...
              <div className="text-gray-400 text-center mt-10">No hay carrera siguiente</div>
            )}
            {/* Decoracion */}
            <div className="absolute bottom-0 left-0 h-2 w-full bg-gradient-to-r bg-gradient-to-br from-red-950/20 to-black/40 shadow-lg">
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent animate-pulse"></div>
