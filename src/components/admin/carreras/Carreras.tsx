        ...
        <div className="container relative mx-auto px-6">
                ...
                <div
                    ...
                    <div className="relative z-10 flex flex-col h-full text-center">
                        <h3 className="text-2xl font-bold text-white">
                            {carrera.nombre || "Carrera"}</h3>
                        <p className="mt-2 text-gray-300 text-md">
                            Fecha: {carrera.fechaCarrera}</p>
                        <p className="mt-2 text-gray-300 text-md">
                            Hora: {carrera.horaCarrera}</p>
                        <p className="mt-2 text-gray-300 text-md">
                            Circuito: {carrera.circuito?.nombre || "-"}</p>
                    </div>
            ...
            {modalEditar && carreraEditando && (
                <ModalCarrera
                    ...
                    pilotos={pilotos}
                    onSubmit={async (carrera) => {
                    const res = await fetch(`http://localhost:3000/carreras/${carrera.id}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify(carrera),
                    });
