...
export default function ModalCarrera({ carrera = {}, circuitos, pilotos, onSubmit, onCancelar }) {
    ...
    const handleSubmit = (e) => {
        ...
        const pilotosCarrera = pilotos.filter(p => pilotosSeleccionados.includes(p.id));
        onSubmit({
        ...carrera,
        nombre,
        fechaCarrera,
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <form
                ...
                <div className="my-2">
                    <label className="block text-gray-300 mb-1">Pilotos participantes:</label>
                    <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto">
                        {pilotos.map(p => (
                        <label key={p.id} className="flex items-center gap-2 text-gray-200">
                            <input
                                type="checkbox"
