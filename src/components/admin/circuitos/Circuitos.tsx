...
export default function Circuitos() {
  ...
  const agregarCircuito = (nuevoCircuito) => {
    ...
    setFeedback({
      open: true,
      mensaje: "¡Circuito agregado correctamente!",
      tipo: "exito",
    });
  };
  ...
  const eliminarCircuito = (id) => {
    ...
    fetch(`http://localhost:3000/circuitos/${id}`, { method: "DELETE" })
      .then((res) => {
        ...
        setFeedback({
          open: true,
          mensaje: "¡Circuito eliminado correctamente!",
          tipo: "exito",
        });
      })
      .catch((err) => {
        console.error("Error al eliminar el circuito:", err);
      });
  };
  ...
  const editarCircuito = (circuitoActualizado) => {
    ...
    setFeedback({
      open: true,
      mensaje: "¡Circuito editado correctamente!",
      tipo: "exito",
    });
  };

  // VER SI SE QUEDA O NO
  // Cierre automático del feedback
  useEffect(() => {
    if (feedback.open) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {circuitos.map((circuito) => (
            ...
            >
              ...
              <div
                className={
                  "absolute bottom-0 left-0 h-2 w-full bg-gradient-to-r from-red-600 to-red-500"
                }
              >
        ...
        {feedback.open && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div className="bg-gradient-to-br from-gray-900 via-black to-gray-900 p-6 rounded-xl shadow-lg border border-red-600/40 max-w-md w-full relative mx-3">
              ...
              <h3
                className={`text-2xl font-bold mb-4 text-center bg-gradient-to-r ${
                  feedback.tipo === "exito"
