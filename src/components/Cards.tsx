...
export function Features() {
  const features = [
    ...
    },
    {
      ...
      description: "Analiza tu rendimiento con métricas avanzadas. Mejora tus predicciones con IA y machine learning.",
      status: "Disponible",
      icon: "📊",
      gradient: "from-green-600 via-green-500 to-green-400",
      bgPattern: "bg-gradient-to-br from-green-950/20 to-black/40",
    },
  ]
      ...
      <div className="container relative z-10">
        ...
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((feature, index) => (
            <Card
              key={index}
              className={`relative overflow-hidden border-2 border-red-900/50 hover:border-red-500/80 transition-all duration-500 hover:shadow-2xl hover:shadow-red-500/20 transform hover:-translate-y-2 ${feature.bgPattern} backdrop-blur-sm`}
            >
              ...
              <CardHeader className="pb-4 relative z-10">
                ...
                <CardTitle className="font-montserrat text-xl font-bold text-white mt-4 leading-tight">
                  {feature.title}
                </CardTitle>
              </CardHeader>
        ...
        <div className="mt-16 text-center">
          <div className="inline-flex items-center gap-4 px-6 py-3 bg-gradient-to-r from-red-950/50 to-black/50 rounded-full border border-red-800/50 backdrop-blur-sm">
            <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
            <span className="text-gray-300 font-medium">Próximas carreras cargándose...</span>
            <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div>
          </div>
        </div>
      </div>
